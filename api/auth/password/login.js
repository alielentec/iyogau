...
export default async function handler(req, res) {
  try {
    ...
    const account = await authenticatePasswordAccount(await readJson(req));
    const user = toSessionUser(account);
    setSessionCookie(req, res, user);
    return sendJson(res, 200, { ok: true, user: { id: user.sub, email: user.email, name: user.name, provider: user.provider } });
  } catch (err) {
